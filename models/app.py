...
<style>
  ...
  .main, .block-container { background-color: #F8F9FB !important; padding-top: 0 !important; }
  ...
  /* KPI CARDS */
  .kpi-card { background: white; border-radius: 12px; padding: 20px 24px; border: 1px solid #E8ECF0; border-left: 4px solid #1A6BFF; }
  .kpi-label { font-size: 11px; color: #6B7C93; font-weight: 500; text-transform: uppercase; letter-spacing: 0.8px; }
  ...
  /* Cacher les boutons de navigation Streamlit */
...
# Navbar HTML
def navbar(current):
    pages = ['Accueil', 'Exploration', 'Analyse Approfondie', 'Prédiction ML']
    links = ""
    for p in pages:
        active = "active" if current == p else ""
        links += f'<span class="nav-link {active}" id="nav-{p}">{p}</span>'
    st.markdown(f"""
    <div class="navbar">
      <div class="navbar-brand">✈️ &nbsp; Flight Delay <span>Intelligence</span></div>
      <div class="navbar-links">{links}</div>
      <div class="navbar-right">50 000 VOLS · RF 93%</div>
    </div>
    """, unsafe_allow_html=True)
...
with col1:
    if st.button("Accueil", key="btn_accueil"):
        ...
        st.rerun()
with col2:
    if st.button("Exploration", key="btn_exploration"):
        st.session_state.page = 'Exploration'
        st.rerun()
with col3:
    if st.button("Analyse", key="btn_analyse"):
        st.session_state.page = 'Analyse Approfondie'
        st.rerun()
with col4:
    if st.button("Prédiction ML", key="btn_prediction"):
